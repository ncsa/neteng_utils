#!/usr/bin/env python3
# NetBox custom script: Allocate reserved NETWORK/GATEWAY(/BROADCAST) IPs per prefix
#
# - Idempotent: avoids re-saving when nothing changed
# - Compares IPv4/IPv6 semantically (host + prefixlen) AND string form
...
class AllocateEdges(Script):
    """
    For each active, non-container Prefix:
      - IPv4: upsert NETWORK (reserved), GATEWAY (reserved), BROADCAST (reserved)
      - IPv6: upsert NETWORK (reserved), GATEWAY (reserved)
    ...
    class Meta:
        name = "Allocate/Update reserved IPs for prefixes (tenant=org_NERD)"
        description = (
            "Creates or updates NETWORK/GATEWAY/BROADCAST (IPv4) and NETWORK/GATEWAY (IPv6) "
            "as reserved for active, non-container prefixes. Tenant forced to org_NERD. "
    ...
    def _role_tags(self):
        return {
            ...
            "GATEWAY": self._get_or_create_tag("gateway", "4caf50"),             # green
            "BROADCAST": self._get_or_create_tag("broadcast-address", "ff9800"), # orange
        }

    def _get_nerd_tenant(self):
        try:
            # If your tenant is identified by slug, use: Tenant.objects.get(slug="org_nerd")
            return Tenant.objects.get(name="org_NERD")
        except Tenant.DoesNotExist:
            raise RuntimeError("Tenant 'org_NERD' does not exist in NetBox!")
    ...
    def _ensure_updated(
        ...
        *,
        ...
        Compare IPv4/IPv6 semantically (host + prefixlen) and also string form.
        Always assign address as the host-form string "<host>/<parent_plen>".
        """
        changed = []
        ...
        # Status is a StatusField; compare slug/string robustly
        if self._status_slug(ip_obj) != "reserved":
            ip_obj.status = "reserved"
            changed.append("status=reserved")
        ...
        # Save before M2M add in edge cases where unsaved state can break .tags.add()
        if role_tag and not ip_obj.tags.filter(pk=role_tag.pk).exists():
            # Save pending field changes so M2M add is safe
            if changed:
                try:
                    ip_obj.save()
                    ...
                    return
                changed = []  # reset; we saved already
        ...
        if changed:
            try:
                ip_obj.save()
                self.log_success(f"  Updated: {ip_obj.address} ({description})")
            except ValidationError as e:
    ...
    def _create_ip(
        ...
        *,
        ...
        try:
            ...
            self.log_success(f"  Created: {ip.address} ({description}, tenant=org_NERD, status=reserved)")
            return ip
        except ValidationError as e:
    ...
    def run(self, data, commit):
        ...
        for pfx in qs.iterator():
            ...
            # Ensure we have a netaddr.IPNetwork for the prefix math
            ...
            # NETWORK (v4 & v6) = the network address itself
            self._upsert_ip(
                vrf=vrf,
                tenant=nerd_tenant,
                description="NETWORK",
        ...
        self.log_success(
            f"Done. Processed prefixes: {processed}. All updates enforce reserved status, org_NERD tenant, descriptions, and tags."
        )
